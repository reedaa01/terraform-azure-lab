...
Projet pratique Terraform (Hands-on Terraform practice project) pour apprendre l'infrastructure Azure pas a pas (step by step).

## Parcours D'apprentissage (Learning Path)

Suivez les dossiers dans l'ordre (Follow the folders in order), des bases jusqu'aux workflows de type production (from basics to production-style workflows).
...
Developper votre maitrise de Terraform sur Azure (Build confidence with Terraform on Azure), des ressources de base a une infrastructure modulaire prete pour la production (from basic resources to modular, production-ready infrastructure).